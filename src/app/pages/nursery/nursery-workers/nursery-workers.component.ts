...
export class NurseryWorkersComponent implements OnInit {
  ...
  ngOnInit() {
    this.workerTypes = [
      {
        name: this.commonService.translate(WebRole.NURSERY_SUPERVISOR, TranslationKey.WEB_ROLE),
        value: WebRole.NURSERY_SUPERVISOR,
      },
      {
        name: this.commonService.translate(WebRole.NURSERY_WORKER, TranslationKey.WEB_ROLE),
        value: WebRole.NURSERY_WORKER,
      },
    ];
    this.buildForm();
    return this.userService
      .getUsers(new Map().set('notWebRole', ['NURSERY_DIRECTOR', 'NURSERY_SUPERVISOR', 'NURSERY_WORKER']))
      .subscribe((res) => {
  ...
  getNurseryWorkersDataloader(params?: Map<string, string[]>) {
    return this.userService.getUsers(
      params.set('webRole', ['NURSERY_DIRECTOR', 'NURSERY_SUPERVISOR', 'NURSERY_WORKER'])
    );
  }
  ...
  onAddWorkers() {
    ...
    this.userService.addRoleToUsers(this.form.value.type.value, this.form.value.workers).subscribe({
      next: (res) => {
        ...
        this.grid.refresh();
        this.popupService.success('Users Successfully added to Nursery Workers!');
      },
      error: () => {
