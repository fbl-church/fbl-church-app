...
export class AwanaAttendanceDetailComponent implements OnInit, OnDestroy {
  ...
  onEditClick() {
    this.navigationService.navigate(`/awana/check-in/${this.record.id}/details/edit`);
  }

  onChildrenCheckInEditClick() {
    this.navigationService.navigate(`/awana/check-in/${this.record.id}/details/children`);
  }
